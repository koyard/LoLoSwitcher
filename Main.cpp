...
//      - Сделана чистка кода, чтобы небыло предупреждений 
...
//        запуск делается от от обычного пользователя, происходит переключение 
...
//        действия по повышению и понижению привелегий
//      - Сделана инверсия переключения привелегий. Раньше программа
//        работала с высокими привелегиями, понижая уровень доступа только
//        при выполнении bash-команд, заданных в опциях CommandX.
//        Теперь программа постоянно работает на низком уровне привелегий,
//        повышая уровень доступа только в моменты считывания данных
//        из файла устройства ввода

// 0.23 - Основана на версии 0.22
//      - Сделано определение STRING_LEN который задает стандартную длинну
...
//      - Обнаружено, что инверсия переключения привелегий имеет побочный
//        эффект. Момент, когда выполняется bash-команда в отдельном
//        треде, может наложиться на момент повышения привелегий
...
//        уровня привелегий отражается как на основном потоке, так и на
...
//        с ограничением на длинну строки. Затем в конец области строки 
//        добавляется 0
...
//        Это необходимо, чтобы LoLoSwitcher вегда работал, даже если клавиатура 
   ...
   if(rb < (int) sizeof(struct input_event)) 
    {
     ...
     // ненужно, а нужно просто пропустить сбойное событие
     /* 
     printf("Short input keyboard device read\n");
     exit(1);
     */

     return 0; // 0 обозначает что никаких событий небыло
    }
